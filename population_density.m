...
for q=1:length(finaldata)
    finaldata(q,7)=log10(finaldata(q,6));
end
...
for i = 1:length(all)
    latInd = find(gridpop(:,1) == all(i,1));
    lonInd = find(gridpop(1,:) == all(i,2));
    if (all(i,1) < 25.75) || (all(i,1) > 48.75) % filter out southern-most and northern-most data
        gridpop(latInd,lonInd) = nan;
    else
        gridpop(latInd,lonInd) = all(i,3);
    end
end
...
usamap conus
plotm(coastlat,coastlon,'k')
contourfm(u_lat,u_lon_east,gridpop_east,'linecolor','none')
scatterm(west_all(:,1),west_all(:,2),40,west_all(:,3),'filled')
bordersm('continental us','k')
cmocean('amp')
cb = contourcbar("southoutside");
title('Population Density in the Contiguous United States (Log Scale)')
cb.XLabel.String = 'Population Density (log_{10}{x})';
hold on
t = plotm(lat_turb,lon_turb,'b.','markersize',5);
leg = legend([t]);
